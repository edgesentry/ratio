...
| テーゼ | 生データを出さずに ODS に参加する（*participate in ODS without shipping raw data*） |
...
- 秘匿・帯域・断続接続のため **生データをローカルに留めつつ** データスペースへ参加したいチーム
...
| 何を作る？ | 取込 → 導出／検証 → 生データとプロダクトの分離 → ODS ハンドオフ |

---

## 議論アジェンダ（初回）

1. **テーゼの確認** — 生データを出さない参加；共有可能プロダクトの中／外
