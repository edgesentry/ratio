...
既定パスのアーキテクチャ: [`ARCHITECTURE.ja.md`](ARCHITECTURE.ja.md)  
想定 PoC 分野（工場／海事）: [`POC.ja.md`](POC.ja.md)  
共有可能プロダクト封筒: [`PRODUCT_ENVELOPE.ja.md`](PRODUCT_ENVELOPE.ja.md)  
ODS ハンドオフ: [`ODS_HANDOFF.ja.md`](ODS_HANDOFF.ja.md)  
...
| すること | しないこと |
|----------|------------|
| 現場で **共有可能プロダクト**（結果＋オントロジー文脈＋利用条件）を作り、**公式 ODS スタック**で登録／発見／Pull 提供する | 生データを ODS の主オファリングとして載せる |
...
| **スコープ** | 正本は [`SCOPE.ja.md`](SCOPE.ja.md)。要約: 薄い取込→導出／検証→分離→公式ハンドオフ。L2/L3/L4 再実装・深い SI・消費者業務・語彙合意は外 |
...
| **共有可能プロダクト** | 上記。Pull 可能な **結果＋意味＋利用条件**（＋来歴）。**生データではない**。DPQM の Data Product＋Ontology Product *関心*を現場で具体化したもの。 |
...
| **エッジ／OT オーナー** | 秘匿、帯域、断続リンク | データスペース・ハンドオフ前の生データの保管と現場導出 |
...
| 2 | 海事 | 同じパイプライン；断続リンク下でメタのみ egress |
...
| 弱い | 強い |
|------|------|
| 「ODS をエッジに拡張」 | 「生データを出さずに ODS に参加する」 |
...
| 「メタデータだけ」（曖昧） | 「共有可能プロダクト＝結果＋意味＋利用条件；生データはローカル」 |
