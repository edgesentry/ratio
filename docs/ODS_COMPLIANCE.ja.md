...
PoC ヘルパー: [`register-openfga-products.sh`](../poc/scripts/ods/register-openfga-products.sh)、[`enable-authzen.sh`](../poc/scripts/ods/enable-authzen.sh)、[`verify-l2-pull.sh`](../poc/scripts/ods/verify-l2-pull.sh)。
...
[ 消費者クライアント ]
  ...
  3) 応答: JSON-LD 共有可能プロダクト（生バイトなし）

[ 提供者（Ratio）]
  共有可能プロダクトを industry API（:8787）へ POST
  L2 ルート /products/** を上流に登録
  JWT 発行・AuthZEN 評価は行わない；生はローカル（R1）
```
...
|--------|----------------|--------------|
| **消費者**（Pull） | AuthZEN 有効時 A1–A4、Z1–Z5；L2 経由で生は期待しない | 公式スタック向け手順・スクリプト；L2/L3 は再実装しない |
| **提供者**（Ratio 現場） | industry API で**共有可能プロダクト**を提供；L2 ルート `/products/**` 登録 | 導出・SHACL・生／プロダクト分離・ハンドオフ（**R2–R4**） |
...
ODS-RAM や ODP の改訂で層／プロトコル名が変わったら、先に **§3 の要件アンカー**と **§6 の URL**を更新する。Ratio のエッジ方針（R1–R4）は分離しておき、準拠のドリフトが「生を出さない」の定義を黙って書き換えないようにする。
