...
2. 生データは船上ストア；プロダクトはリンク復旧時のみ ODS 向けにキュー  
3. 可能な限り工場側と同じ共有可能プロダクト形状（可搬パイプライン）  
4. 接続時に ODS SDK で参加  
...
|----|------|------|--------|--------------|----------|------|
| RB1 | デバイス取込 | **Reuse** GW/スタブ + **Build** 薄い消費 | WoT TD；既存 IIoT／WoT GW | 薄い TD 読込（`--td`）；マルチプロトコル GW は作らない | `examples/td/` のスタブ TD | 分野ごとに系統1本をロック |
...
| ベンダー／セル／船 | 未定（スタブ＋録画／合成の生データで進める） |
