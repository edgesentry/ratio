...
関連: [`DISCUSSION.ja.md`](DISCUSSION.ja.md) · [`ODS_COMPLIANCE.ja.md`](ODS_COMPLIANCE.ja.md) · [`ODS_HANDOFF.ja.md`](ODS_HANDOFF.ja.md) · [`LAYOUT.ja.md`](LAYOUT.ja.md)
...
```
[ OT 現場 ] --(薄い TD 消費)--> [ Ratio: 導出・SHACL・分離・引き渡し ]
                                      │ 共有可能プロダクトのみ
                                      ▼
                               [ 公式 ODS: L3 / L2 / L4 / … ]
                                      │ Pull
...
`ratio-serve` は **公式の代替ではない**（L2 上流 industry API の仮置き）。

---

## ② Ratio のスコープ（公式に無い「発生点の糊」）

パイプライン: **取込（薄い）→ 導出／検証 → 生データとプロダクトの分離 → 公式への引き渡し**。

| 含む | 含まない（誤解しやすいもの） |
|------|------------------------------|
| 共有可能プロダクトの書式（結果＋オントロジー文脈＋利用条件） | 生データを ODS で主に提供すること |
...
| 生データのローカル保管指針と `local://` ポインタ方針 | ハードリアルタイム制御 |
| Oxigraph 等 OSS による検証オーケストレーション | ODP／L2／L3／L4 の再実装 |
| 公式スタック（または仮 industry URL）への引き渡し | 消費者向け業務 UI／本番 Agentic 製品 |

### 語彙・TD・消費者（2 / 3 / 4）

| # | 領域 | Ratio | 説明 |
|---|------|-------|------|
| **2** | 語彙／SHACL | **最小のみ** | 書式＋共通 SHACL＋PoC shapes は Ratio。業界合意・巨大語彙ガバナンスは外 |
| **3** | WoT TD／取込 | **薄い層のみ** | TD 消費・スタブ・1系統アダプタは Ratio。深いベンダー SI は外 |
| **4** | 参照 Pull 消費者 | **コアに含めない** | サンプル: `ratio-pull`（[`samples/`](../samples/)）。本番アプリは外。Ratio 本体の責務外 |
...
| **参照 Pull クライアント（任意）** | Ratio チームまたはコミュニティが **別成果物**として | サンプル: `ratio-pull`。Ratio コアに吸収しない |
